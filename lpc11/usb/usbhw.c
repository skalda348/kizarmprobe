...
void USB_EPInit (void) {
  ...
  *addr++ = (USB_MAX_PACKET0<<16) | ( (uint16_t) (data_ptr>>6));
  EPList[j].buf_ptr = data_ptr;
  EPList[j++].buf_length = USB_MAX_PACKET0;
  data_ptr += USB_MAX_PACKET0;  /* Őâ¸ö64×Ö˝Ú¸řEP0 IN buffer */
...
uint32_t USB_ReadSetupEP (uint32_t EPNum, uint8_t *pData) {
  ...
  for (n = 0; n < (cnt + 3) / 4; n++) {
    // Tohle Cortex-M0 a gcc nezkousne ani náhodou (unaligned access). Použijeme memcpy()
    //* ( (__packed uint32_t *) pData) = * ( (__packed uint32_t *) dataptr);
    memcpy (pData, dataptr, 4);
    pData += 4;
    dataptr++;
  }
        ...
        if ( (n & 1) == 0) {                /* OUT Endpoint */
          if (n == 0) {                     /* Control OUT Endpoint */
            if (LPC_USB->DEVCMDSTAT & USB_SETUP_RCVD) {
              ...
              if (EpHandlers[0].pEpn) {
                EpHandlers[0].pEpn (0, USB_EVT_SETUP);
                LPC_USB->DEVCMDSTAT |= USB_SETUP_RCVD;
                continue;
              }
            }
          }
          if (EpHandlers[m].pEpn) {
            EpHandlers[m].pEpn (EpHandlers[m].data, USB_EVT_OUT);
          }
        } else {                            /* IN Endpoint */
          if (EpHandlers[m].pEpn) {
            EpHandlers[m].pEpn (EpHandlers[m].data, USB_EVT_IN);
          }
        }
      }
    }
