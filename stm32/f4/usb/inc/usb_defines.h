...
static inline volatile uint32_t USB_OTG_READ_REG32 (volatile uint32_t * reg) {
  return * reg;
}
static inline void USB_OTG_WRITE_REG32  (volatile uint32_t * reg, uint32_t value) {
  * reg = value;
}
static inline void USB_OTG_MODIFY_REG32 (volatile uint32_t * reg, uint32_t clear_mask, uint32_t set_mask) {
    USB_OTG_WRITE_REG32(reg, (((USB_OTG_READ_REG32(reg)) & ~clear_mask) | set_mask ));
}
